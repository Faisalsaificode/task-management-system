...
const LoginPage = ({ userType, onLogin, onSwitchType, onSwitchToRegister }) => {
    ...
    try {
      ...
      if (data.success) {
        if (data.user.role !== userType) {
          setAlert({ 
            message: `Access denied. ${userType} credentials required.`, 
            type: 'error' 
          });
          setLoading(false);
          return;
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4">
        <div className={`${bgGradient} text-white p-8 rounded-t-2xl text-center`}>
          <div className="bg-white bg-opacity-20 rounded-full w-20 h-20 flex items-center justify-center mx-auto mb-4">
            {userType === 'admin' ? <Users size={40} /> : <CheckCircle size={40} />}
          </div>
          <h2 className="text-3xl font-bold mb-2">
            {userType === 'admin' ? 'Admin' : 'User'} Login
          </h2>
          <p className="text-purple-100">Task Management System</p>
        </div>
